          ...
          <article key={`node-${nodeKey}`}>
            ...
            <div className="mt-5 space-y-3">
              {node.paragraphs.map((p, i) => (
                <p
                  key={i}
                  className={`rise font-serif text-lg leading-normal ${
                    p.startsWith("«")
...
function Hud({
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-30 border-b border-(--color-smoke) bg-(--color-abyss)/85 backdrop-blur-md">
      <div className="mx-auto flex max-w-3xl items-center gap-5 px-6 py-2.5 sm:gap-8">
        <button
          onClick={onMenu}
  ...
  return (
    <main className="veil relative flex min-h-dvh items-center justify-center overflow-hidden px-6">
      ...
      <div className="relative z-10 mx-auto max-w-2xl text-center">
        ...
        <p
          className="rise mt-6 font-mono text-xs tracking-[0.3em] text-(--color-ash)/80 uppercase"
